...
The metadata derived for each Document: `incident_type`, `doc_domain`, `tags`, `asset_scope`. (`doc_type` comes from the Document loader's format table.) It comes only from the Document's **relative path**, split into whole-word tokens. Content, the absolute path and the repo location never affect it. Filenames are therefore load-bearing.

## Document loader
The single table of supported file extensions, each paired with its reader and its `doc_type`. The ingest allowlist is derived from it.
